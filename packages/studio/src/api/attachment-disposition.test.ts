...
describe("attachmentDisposition", () => {
  it("keeps Chinese filenames ASCII-safe and includes RFC 5987 filename*", () => {
    ...
    expect(value).toBe(`attachment; filename="_.txt"; filename*=UTF-8''${encodeURIComponent("醉词.txt")}`);
  });
  ...
  it("replaces a fully non-ASCII name with a single underscore fallback", () => {
    const value = attachmentDisposition("醉词");
    expect(value.startsWith('attachment; filename="_"')).toBe(true);
    expect(value).toContain(`filename*=UTF-8''${encodeURIComponent("醉词")}`);
    expect(() => new Headers({ "Content-Disposition": value })).not.toThrow();
  });

  it("falls back to download when the name is empty", () => {
    const value = attachmentDisposition("");
