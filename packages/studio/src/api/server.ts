/**
 * FantaWriter 2.0 P0 (2026-09-01): health/shutdown/lock force-release,
 * truth PUT takes the book lock, bind 127.0.0.1 with an explicit port.
 * Upstream: InkOS v1.8.0 studio server.
 */
...
export function createStudioServer(initialConfig: ProjectConfig, root: string, overrides: { readonly nodeImageGenerator?: NodeImageDeps } = {}) {
  ...
  app.get("/api/v1/health", (c) => {
    return c.json({
      ...
      engine: "inkos-studio",
      projectRoot: root,
      instanceToken: process.env.FW_INSTANCE_TOKEN ?? null,
      pid: process.pid,
    });
  });
  ...
  app.get("/api/v1/books/:id/lock", (c) => {
    const id = c.req.param("id");
    if (!isSafeBookId(id)) {
      throw new ApiError(400, "INVALID_BOOK_ID", `Invalid book ID: "${id}"`);
    }
    const owner = state.inspectBookLock(id);
    return c.json({ locked: Boolean(owner), owner });
  });

  app.post("/api/v1/books/:id/lock/force-release", async (c) => {
    const id = c.req.param("id");
    if (!isSafeBookId(id)) {
      throw new ApiError(400, "INVALID_BOOK_ID", `Invalid book ID: "${id}"`);
    }
    ...
    const owner = await state.forceReleaseBookLock(id);
    broadcast("book:lock-released", { bookId: id, owner });
    return c.json({ ok: true, released: true, owner });
  });
