...
export function findProjectRoot(): string {
  const explicit = process.env.INKOS_PROJECT_ROOT?.trim();
  if (explicit) return resolve(explicit);
  if (process.env.INKOS_DESKTOP === "1" || process.env.FW_REQUIRE_EXPLICIT_ROOT === "1") {
    throw new Error(
      "Desktop/engine requires explicit INKOS_PROJECT_ROOT; refuse process.cwd().",
    );
  }
  return process.cwd();
}
