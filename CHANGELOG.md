...
- 一次作品读取失败不再整页改成「引擎连不上，请重启应用」。已有落笔内容保留，可重新连接；HTTP 错误与上游失败分开提示。
- 连接恢复每次最多补读一次，健康探测不再广播会自激重试的 ready；SSE 重连后合并补查工作区。
- 后台创作任务失败收尾出错时不再拖垮引擎；失败记录无法写入时，查询仍返回已停止/异常。运行 JSON 改为同目录一次覆盖。
- Windows 唤醒不杀仍存活的已接管引擎；原端口不能恢复时进入需人工处理，不宣布 ready。织卷审查改为后台任务，响应丢失时核对原任务。
- 历史版本在请求未完成时关闭再打开，仍显示当前选中版本的正文。

## [2.2.7] - 2026-09-21

- 修复织卷采用后直接进入落笔仍显示「还没有可写的章」的问题；采用成功立即刷新创作阶段，防止旧请求覆盖新状态。
- 落笔等待阶段信息时不再提前显示缺少规划的提示。已有采用结果可直接继续使用，无需重新生成或采用。

## [2.2.6] - 2026-09-20

- 修复织卷审查后修订误用全书范围、单次输出过长导致 JSON 不完整的问题。修订默认当前卷，仅处理已有概要，每批最多四章。
- 修订改为后台执行，显示实际进度与错误；每批保存候选，暂停或失败后可继续剩余修订，沿用原审查意见与作者要求，不覆盖正式采用稿。

## [2.2.5] - 2026-09-20

- 修复织卷章概要中正文英文双引号未转义导致的 JSON 解析失败，保留原引语内容；兼容仅用于织卷正文类字段。
- 保留缺失字段、结构损坏和截断输出的原错误处理，不补造章节。已完成的概要仍可通过「继续剩余范围」保留并续写。

## [2.2.4] - 2026-09-20

- 织卷重分卷按原卷归属保留后卷作者注记；新书采用必须有连续分卷，不能零卷或跳号。
- 长设定条目优先摘录「核心约束」和别名，避免只取文档开头而漏掉关键事实。
- 落笔缺本章规划时返回中文 400；修订失败记独立 run。本地安装包 `Inkborne-Setup-2.2.4.exe`。

## [2.2.3] - 2026-09-17

- 修复 Claude Opus 5 等模型在确认建书、基础设定审查和大纲生成时因内部温度覆写而返回 400 的问题。统一在请求发送前省略这些模型已弃用的 temperature、top_p、top_k，覆盖普通生成及工具调用。
- 保留支持采样参数的模型原有行为；不需要更换已配置的创作和审查模型。

## [2.2.2] - 2026-09-17

- 检查更新先查本地安装包，找到比当前版本更新的安装包即可安装，不再被 GitHub 上的旧版本遮蔽；本地没有新版时继续检查远端。
- 修复 Windows 桌面重定向后更新包投放到错误目录的问题；发布时同时投放新品牌文件名及旧版兼容文件名。

## [2.2.1] - 2026-09-16

中国水墨界面与四阶段创作更新。Windows 安装包：`Inkborne-Setup-2.2.1.exe`，保留两套旧文件名前缀兼容更新。许可证、InkOS 署名、应用标识和作品存储路径保持不变。

### 界面与交互

- 首页只保留作者头像、名字、书法四阶段名称和「进入问心」；导入、模型、外观与旧工具归入统一配置入口。
- 没有作品时显示引导，一本作品时恢复创作位置，多本作品时显示封面书架；第二本书沿用作者信息。
- 问心以聊天为主，生成正典后可展开对照；研墨、织卷以目录与成果为主；落笔提供可收起的章节目录并记住选章。
- 文稿默认只读，主动编辑后保存；编辑中离开可保存、放弃或继续编辑，取消离开不再提前清空会话。
- 审查侧栏与文稿并排，重生窗口可填写保留内容和修改重点；历史稿先预览，再明确恢复为候选。保存、审查与采用保持独立。
- 新增近白／深炭灰主题、可换纸色、本地书法字体与留白处的水墨装饰。

### 稳定性

- 研墨补全只生成缺失成果；批量报告的修订范围按所选意见实际目标显示并再次校验版本。
- 生成要求传递到四阶段任务，织卷暂停续跑保留修改要求；正典元数据正确往返保存引号和换行。
- 补齐桌面静态资源处理、构建产物依赖和严格测试类型检查，保留明确项目根与桌面壳分配端口。

以下四阶段能力和修复也随本次安装包一并交付：
...
- 重分卷会保留已有章概要；非法分卷不能完成或采用；分卷审查修订会改 volumes。
- 结构采用失败时正式卷纲与 manifest 一起回滚；新四阶段书不能再直写已采用卷纲。
- 落笔按当前章概要挑选设定；没有已采用本章概要时拒绝自动生成。
- 织卷全书目标取自正典，不再把本次结束章当成全书篇幅；可先生成分卷规划再写章概要。
- 新研墨目录书不再显示旧七项「待补充」；未采用设定不会进入织卷上下文。
- 织卷 JSON 非法转义会做一次安全修复，仍失败则明确报错。
