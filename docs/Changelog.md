...
落笔读取失败可恢复，不再把浏览器网络错误一律写成必须重启。连接恢复每次最多补读一次；SSE 重连合并补查工作区。后台失败无法落盘时查询仍返回已停止。运行 JSON 同目录一次覆盖。唤醒不杀仍存活的已接管引擎；原端口不能恢复则需人工处理。织卷审查走已有后台 run，响应丢失时核对原任务。历史窗口关闭后重开同一在途版本仍显示正文。安装包 `Inkborne-Setup-2.2.10.exe`。未证明已消除所有现场随机断连。
...
用户向说明见根目录 [CHANGELOG.md](../CHANGELOG.md)。Windows 安装包 `Inkborne-Setup-2.1.4.exe`（#107 P0-6 墨·宣·朱配色 + StageDot + 文案；#108 P0-7 chrome 上提 + 页头对话框；#109 P1-5 深页排版：落笔/研墨/织卷/章页等）。
...
用户向说明见根目录 [CHANGELOG.md](../CHANGELOG.md)。Windows 安装包 `Inkborne-Setup-2.1.1.exe`（#98 首页只读作者信息 + 封面书架；侧栏去掉在创、两列排列；对谈记录改问心记录；日志呈现为 AI 动态；守护进程不变；问心与对谈统一；去掉顶栏中英文切换，语言进项目设置）。
...
用户向说明见根目录 [CHANGELOG.md](../CHANGELOG.md)。Windows 补丁安装包 `FantaWriter-Setup-2.0.12.exe`（#84 短篇分阶段：先大纲确认再按章写；工具卡中文阶段可见；短篇进作品列表可打开阅读）。
